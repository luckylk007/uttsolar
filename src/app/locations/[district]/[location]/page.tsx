...
export default async function LocationDetailPage({ params }: LocationPageProps) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10 space-y-12 sm:space-y-16">
      ...
      <section className="bg-white p-6 sm:p-8 rounded-2xl sm:rounded-3xl border border-[#E1E8DE] shadow-xs space-y-4">
        <h3 className="text-base font-bold text-[#122417]">
        ...
        <div className="flex flex-wrap gap-2">
          {location.nearbyAreas.map((area) => (
            <span
              key={area}
              className="px-3 py-1 bg-[#F4F7F2] border border-[#E1E8DE] rounded-full text-xs font-medium text-[#122417]"
            >
              📍 {area}
            </span>
          ))}
